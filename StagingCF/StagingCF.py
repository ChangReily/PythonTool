...
def ParsingIniFile():
    ...
    if ReleaseType is '':
        ...
        if (ReleaseType != 'ProductionRelease') and (ReleaseType != 'TestRelease'):
...
def CheckBiosVersionAlreadyUse():
    ...
    print (f'\n===== Check BIOS version already or not =====')
    # Convert Bios version to a list
    BiosVersionList=BiosVersion.split('.')
    ...
    Processouts, Processerrs = Process.communicate()
    if  Process.returncode != 0:    
        print (f'BIOS Version is not used.!!')
        Buffer = Processerrs.decode('UTF-8','ignore')
        print ('----------')
        print (Buffer)
        print ('----------')
...
def CreateStageBranch():
    ...
    if Version == '':
        SvnCmd=f'svn copy -m "Perpare {ProjectName} {BiosVersion} signing with {ReleaseType}" --parents {ProjectUrl} {PrepareStagingUrl}'
    else:
        SvnCmd=f'svn copy -m "Perpare {ProjectName} {BiosVersion} signing with {ReleaseType}" --parents {ProjectUrl}@{Version} {PrepareStagingUrl}'
...
if __name__ == '__main__':
    ...
    CallSubprocess(SvnCmd)
    print ('----------')
    print (f'Please checkout the code with above the URL!!')
    print (f'Before send the signing request, please verify local build status!!')
    print ('\n\n')
    os.system("pause")
